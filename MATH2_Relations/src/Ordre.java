...
public class Ordre extends RelationAbstraite {
	...
	public Ordre(Ordre or) {
		...
		// Clone profond de la relation d'ordre
	...
	public void ajouter(Couple c) {
		...
		if (c == null) throw new IllegalArgumentException();
		if (!couples.depart().contient(c.getX()) || !couples.depart().contient(c.getY())) {
			throw new IllegalArgumentException();
		}
		//  On clone la relation actuelle
		Relation tmp = couples.clone();
		//  On ajoute le couple dans la copie
		tmp.ajouter(c);
		//  On ferme en réflexivité et transitivité
		tmp.cloReflex();
		tmp.cloTrans();
		// On vérifie l’antisymétrie
		if (!tmp.antisymetrique()) {
			throw new IllegalArgumentException();
		}
		//Si tout est bon, on valide le changement
		couples = tmp;
	}
